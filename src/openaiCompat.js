...
export function routeKeyFromRequest(req, body, promptBlocks, affinityPrefixChars = 4096) { const headers = req.headers ?? {}; const directHeaders = [headers['x-acp-routing-key'], headers['x-prompt-cache-key']].filter(Boolean); if (directHeaders.length) return String(Array.isArray(directHeaders[0]) ? directHeaders[0][0] : directHeaders[0]); const candidates = [body?.prompt_cache_key, body?.promptCacheKey, body?.cache_key, body?.cacheKey, body?.user, body?.session_id, body?.sessionId, body?.conversation_id, body?.conversationId, body?.thread_id, body?.threadId, body?.metadata?.routing_key, body?.metadata?.routingKey, body?.metadata?.prompt_cache_key, body?.metadata?.promptCacheKey, body?.metadata?.session_id, body?.metadata?.sessionId, body?.metadata?.conversation_id, body?.metadata?.conversationId, body?.metadata?.thread_id, body?.metadata?.threadId, body?.metadata?.user]; for (const candidate of candidates) if (candidate != null && String(candidate).length) return String(candidate); const prefix = promptTextForUsage(promptBlocks).slice(0, affinityPrefixChars); return crypto.createHash('sha256').update(prefix).digest('hex'); }
